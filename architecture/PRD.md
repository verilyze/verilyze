...
| **FR‑019** | Provider selection CLI | `--provider <name>` forces a specific provider; invalid name ⇒ exit 2. | `spd scan --provider osv` uses OSV; `spd scan --provider nvd` uses NVD (when built with `nvd` feature). |
