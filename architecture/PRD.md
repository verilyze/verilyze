...
| **NFR-012** | Test coverage | Unit tests + integration tests must achieve **>= 70% branch coverage** and **>= 90% functional coverage** (including doctest examples). | Coverage is measured with **cargo-llvm-cov** using the **nightly** toolchain with `--branch`; the report is emitted in **Cobertura XML** so CI/CD (e.g. GitHub Actions) can consume it. Run `cargo +nightly llvm-cov --workspace --branch --cobertura --output-path reports/cobertura.xml` (or `make coverage`). Exact commands are in CONTRIBUTING (DOC-007). |
...
| **DOC-007** | Testing & coverage guidelines | Instructions for running the full test suite, interpreting coverage reports (**cargo-llvm-cov**, Cobertura XML), and guidelines for adding new unit/integration tests. Test placement: unit tests in the same file as the code under test (or same crate) in a `#[cfg(test)] mod tests` block; integration tests in `tests/` or equivalent (e.g. binary tests that run the executable). Tests must make the expected behavior they verify clear (e.g. test name, doc comment, or requirement ID). | CI badge shows "Coverage >= 70% branch, >= 90% functional." |
