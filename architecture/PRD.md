...
| **OP-009** | Cache expiration | Each key in the CVE cache has an `expires_at` timestamp. The configuration key is **cache_ttl_secs** (value in seconds); the default is **5 days** (432000 seconds). Expired rows are treated as cache misses on read and refreshed on demand. Physical removal of expired entries from the store is best-effort (e.g. on database init or on write). | After 5 days, a subsequent scan re-queries the provider for the same package. |
