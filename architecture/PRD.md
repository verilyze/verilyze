...
| **FR‑005** | Manifest discovery (initial)** | Detect all registered language plugins (Python, initially). | `spd list` prints `python`. |
...
| **FR‑018** | Database‑listing CLI | `spd db list-providers` enumerates supported CVE providers. | Output includes `osv`. |
...
| **NFR-012** | Test coverage | Unit tests + integration tests must achieve **>= 70% branch coverage** and **>= 90% functional coverage** (including doctest examples). | `cargo tarpaulin --skip-clean --out xml` reports the required percentages. |
...
| **OP-009** | Cache expiration | Each key in the CVE cache has an `expires_at` timestamp. The default TTL is **5 days**; the TTL is configurable <seconds>. Expired rows are treated as cache misses and refreshed on demand. | After 5 days, a subsequent scan re-queries the provider for the same package. |
...
| **OP-011** | Environment-variable naming | All configuration keys are exposed as **SPD_<UPPER_SNAKE_CASE> (e.g., SPD_CACHE_TTL, SPD_PARALLEL_QUERIES).** | Setting `export SPD_PARALLEL_QUERIES=20` influences the run. |
...
| **CFG-005** | Environment-variable overrides | Every configuration key is also exposed as an environment variable prefixed with **SPD_** (e.g., `SPD_CACHE_TTL`). Environment variables have higher predence than any configuration file, but lower precedence than explicit CLI flags. | Setting `export SPD_PARALLEL_QUERIES=30` changes the parallelism even if the same key is defined in a config file. |
| **CFG-006** | Command-line overrides | All configuration options are individually addressable via CLI flags (e.g., --cache-ttl, --parallel-queries). CLI flags have the highest precedence and override everything else. | Running `spd scan --parallel-queries 40` forces the value to 40 regardless of any config file or env-var. |
...
| **MOD-002** | Public trait contracts | Each crate shall expose a small set of **public traits** that define the contract for plug-ins: - ManifestFinder (method fn find(&self, root: &Path) -> Result<Vec<PathBuf>, FinderError>). - Parser (method fn parse(&self, manifest: &Path) -> Result<DependencyGraph, ParserError>). - Resolver (method fn resolve(&self, graph: &DependencyGraph) -> Result<Vec<Package>, ResolverError>). - CveProvider (method async fn fetch(&self, pkg: &Package) -> Result<Vec<CveRecord>, ProviderError>). - DatabaseBackend `#[async_trait] pub trait DatabaseBackend: Send + Sync { async fn init(&self) -> Result<(), DatabaseError>; async fn get(&self, pkg: &Package) -> Result<Option<CveRecord>, DatabaseError>; async fn put(&self, pkg: &Package, rec: CveRecord) -> Result<(), DatabaseError>; async fn stats(&self) -> Result<DatabaseStats, DatabaseError>; async fn verify_integrity(&self) -> Result<(), DatabaseError> { Ok(()) } }`. - Reporter (method fn render(&self, data: &ReportData) -> Result<ReportOutput, ReportError>). - IntegrityChecker (method fn verify(&self, db: &dyn DatabaseBackend) -> Result<IntegrityResult, IntegrityError>). | Adding a new language, database backend, CVE provider, integrity checker algorighm, or report format only requires implementing the relevant traits in a separate crate; the core binary can load it without recompilation. |
...
| **DOC-003** | Configuration reference | Detailed documentation of every configuration key, its default value, accepted types, corresponding environment variable (`SPD_<NAME>`), and CLI flag that overrides it. Presented in a table for quick lookup. | Running `spd config --list` prints the same table as the docs. |
...
| **DOC-007** | Testing & coverage guidelines | Instructions for running the full test suite, interpreting coverage reports (`cargo tarpaulin`), and guidelines for adding new unit/integration tests. | CI badge shows "Coverage >= 70% branch, >= 90% functional." |
