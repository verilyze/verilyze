...
use clap::Parser;
use spd::cli::Cli;
use spd_db::{DatabaseBackend, Package};
...
fn run_async(args: &[&str]) -> i32 {
    ...
    rt.block_on(spd::run(args)).unwrap_or(2)
}
...
fn run_db_show_with_cached_entry_and_raw_vulns() {
    ...
    temp_env::with_var("XDG_CACHE_HOME", Some(p.as_str()), || {
        temp_env::with_var("XDG_DATA_HOME", Some(p.as_str()), || {
            temp_env::with_var("XDG_CONFIG_HOME", Some(p.as_str()), || {
                let path = spd::config::default_cache_path();
                if let Some(parent) = path.parent() {
                    std::fs::create_dir_all(parent).ok();
                }
                let backend =
                    spd_db_redb::RedbBackend::with_path(path, 3600).expect("create backend");
                let rt = tokio::runtime::Runtime::new().expect("runtime");
                rt.block_on(async {
...
fn run_cache_db_path_is_directory_exits_2() {
    ...
    with_temp_xdg(|| {
        ...
        temp_env::with_var("SPD_CACHE_DB", Some(cache_dir.as_os_str()), || {
...
fn run_scan_resolver_fails_exits_2() {
    ...
    with_temp_xdg(|| {
        ...
        std::fs::write(dir.path().join("requirements.txt"), "pkg==1.0\n").expect("write");
        let root = dir.path().to_str().unwrap();
        spd::registry::clear_resolvers();
        spd::registry::register(spd::registry::Plugin::Resolver(Box::new(
            spd::mocks::FailingResolver::new(),
        )));
...
fn run_scan_cve_provider_fails_logs_error() {
    ...
    with_temp_xdg(|| {
        ...
        std::fs::write(dir.path().join("requirements.txt"), "pkg==1.0\n").expect("write");
        let root = dir.path().to_str().unwrap();
        spd::registry::clear_providers();
        spd::registry::register(spd::registry::Plugin::CveProvider(Box::new(
            spd::mocks::FailingCveProvider::new(),
        )));
...
fn run_db_verify_backend_fails_exits_1() {
    let _ = env_logger::try_init();
    with_temp_xdg(|| {
        spd::registry::clear_db_backends();
        spd::registry::register(spd::registry::Plugin::DatabaseBackend(Box::new(
            spd::mocks::FailingDbBackend::new(),
        )));
...
fn run_scan_invalid_language_regex_exits_2() {
    ...
    with_temp_xdg(|| {
        let dir = tempfile::tempdir().expect("tempdir");
        let root = dir.path().to_str().unwrap();
        let cfg_path = dir.path().join("spd.toml");
        std::fs::write(
            &cfg_path,
            r#"[python]
regex = "[invalid"
...
fn run_fp_ignore_db_path_is_directory_exits_2() {
    ...
    with_temp_xdg(|| {
        ...
        temp_env::with_var("SPD_IGNORE_DB", Some(ignore_dir.as_os_str()), || {
